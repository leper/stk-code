...
float Distribution(float roughness, vec3 normal, vec3 halfdir)
{
    ...
    float NdotH = clamp(dot(normal, halfdir) - 0.0001, 0., 1.);
...
vec3 SpecularBRDF(vec3 normal, vec3 eyedir, vec3 lightdir, vec3 color, float roughness)
{
    ...
    // Microfacet model
    return Fresnel(eyedir, H, color) * ReducedGeometric(lightdir, eyedir, normal, roughness) * Distribution(roughness, normal, H) / 4.;
}