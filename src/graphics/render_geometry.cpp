...
{
    // Render Subsurface scattering like for grass
    ...
    if (irr_driver->useAZDO())
    {
        DepthHandle = glGetTextureSamplerHandleARB(getDepthStencilTexture(), MeshShader::GrassSubsurfaceScatteringShader::getInstance()->SamplersId[1]);
...

    {
...

        {
            ...
            glUseProgram(MeshShader::GrassSubsurfaceScatteringShader::getInstance()->Program);
            if (irr_driver->hasARB_base_instance())
            ...
            for (unsigned i = 0; i < meshes.size(); i++)
            {
                ...
                if (UserConfigParams::m_azdo)
                    HandleExpander<MeshShader::GrassSubsurfaceScatteringShader>::Expand(mesh.TextureHandles, STK::make_tuple(0), DepthHandle);
                else
                    TexExpander<MeshShader::GrassSubsurfaceScatteringShader>::ExpandTex(mesh, STK::make_tuple(0), irr_driver->getDepthStencilTexture());
                MeshShader::GrassSubsurfaceScatteringShader::getInstance()->setUniforms(STK::tuple_get<1>(meshes.at(i)), STK::tuple_get<2>(meshes.at(i)), windDir, irr_driver->getSunDirection());
        ...
        if (UserConfigParams::m_azdo)
        {
            glUseProgram(MeshShader::InstancedGrassSubsurfaceScatteringShader::getInstance()->Program);
            ...
            {
                HandleExpander<MeshShader::InstancedGrassSubsurfaceScatteringShader>::Expand(nulltex, STK::make_tuple(0), DepthHandle);
                MeshShader::InstancedGrassSubsurfaceScatteringShader::getInstance()->setUniforms(windDir, irr_driver->getSunDirection());
                glMultiDrawElementsIndirect(GL_TRIANGLES, GL_UNSIGNED_SHORT,
        ...
        else if (irr_driver->hasARB_draw_indirect())
        {
            ...
            {
                GLMesh *mesh = meshes[i];
                TexExpander<MeshShader::InstancedGrassSubsurfaceScatteringShader>::ExpandTex(*mesh, STK::make_tuple(0), irr_driver->getDepthStencilTexture());
                MeshShader::InstancedGrassSubsurfaceScatteringShader::getInstance()->setUniforms(windDir, irr_driver->getSunDirection());
